<?php
  //file: /view/messages/messages_es.php
  $i18n_messages =
  array(
    "title" => "FitnesSuite",
    "An exception occured!" => "Se ha producido un mal comportamiento!",
    "is mandatory" => "es obligatorio",
    "of" => "de",
    "for" => "para",
    "to" => "a",
    "Available resources" => "Recursos disponibles",
    "Practice gives the" => "La práctica da los",
    "Perfect results" => "Resultados perfectos",
    "Get healthy with FitnesSuite, because feeling good is being fit. In the end this is about living well" =>"Ponte en forma con FitnesSuite, porque sentirse bien es estar en forma. Al fin y al cabo, se trata de vivir bien",
    "For further information, please do not hesitate to get in contact with us" => "Si tienes alguna duda o pregunta no dudes en ponerte en contacto con nosotros y te atenderemos gustosamente",
    "To ask us or send us your comments, simply fill in the form and the FitnesSuite team will contact you to discuss your question" => "Para preguntarnos o enviarnos tus comentarios, simplemente cumplimenta el formulario y el equido de FitnesSuite se pondrá en contacto contigo para tratar tu consulta",
    "Create Table" => "Crear tabla",
    "Reservations" => "Reservas",
    "Home" => "Inicio",
    "My profile" => "Mi perfil",
    "Profile" => "Perfil",
    "Search" => "Buscar",
    "About us" => "Sobre nosotros",
    "Trainers" => "Entrenadores",
    "Our trainers" => "Nuestros entrenadores",
    "Classes" => "Clases",
    "Activities" => "Actividades",
    "Phone" => "Teléfono",
    "Pricing" => "Precios",
    "minutes" => "minutos",
    "Confirm User" => "Confirmar usuario",
    "customized" => "personalizada",
    "Workout Tables of this user" => "Tablas de entrenamiento para este usuario",
    "Manage Exercices" => "Gestión de ejercicios",
    "New user added to the app, please, confirm." => "Nuevo usuario añadido a la aplicación, por favor, confirme.",
    "See all activities" => "Ver todas las actividades",
    "Mail successfully sended." => "Mail enviado satisfactoriamente",
    "user successfully updated." => "usuario actualizado satisfactoriamente",
    "user successfully deleted." => "usuario modificado satisfactoriamente",
    "table successfully modified." => "tabla modificada satisfactoriamente",
    "successfully modified." => "modificado satisfactoriamente.",
    "table successfully added." => "tabla añadida satisfactoriamente",
    "table successfully configured." => "tabla configurada satisfactoriamente",
    "Table successfully deleted." => "Tabla borrada satisfactoriamente.",
    "Activity successfully updated." => "Actividad actualizada satisfactoriamente",
    "Activity successfully added." => "Actividad añadida satisfactoriamente.",
    "athlete(s) successfully asigned." => "Atleta(s) asignado satisfactoriamente",
    "Error sending mail." => "Error enviando mail",
    "successfully deleted." => "borrado satisfactoriamente.",
    "Activity successfully deleted" => "Actividad borrada satisfactoriamente.",
    "Reserve successfully deleted." => "Reserva eliminada satisfactoriamente.",
    "Reserve successfully added." => "Reserva añadida satisfactoriamente.",
    "Resource successfully deleted." => "Recurso borrado satisfactoriamente.",
    "You must write a duration"=> "Debe introducir una duración",
    "You must write a number of repetitions" => "Debe introducir un número de repeticiones",
    "You must write a number of series" => "Debe introducir un número de series",
    "ALL CLASSES" => "TODAS LAS CLASES",
    "MEMBERSHIP PRICES" =>"OFERTAS",
    "Add user" => "Añadir usuario",
    "Next events" => "Próximos eventos",
    "Opening hours" => "Horario",
    "Discount of for all members" => "Descuento para miembros",
    "Without description" => "Sin descripción",
    "Please enter your message to notify at group" =>"Por favor, introduzca su mensaje para notificar al grupo",
    "Discount on services and treatments at the GymBase for all membership cards holders."=> "Descuento en servicios y tratamientos de nuestra tienda para todos los titulares de tarjetas.",
    "Monday" => "Lunes",
    "Tuesday" => "Martes",
    "Wednesday" => "Miércoles",
    "Thursday" => "Jueves",
    "Friday" => "Viernes",
    "Saturday" => "Sábado",
    "Sunday" => "Domingo",
    "Enjoy it!" => "¡Disfrútalo!",
    "Pricing and plans" => "Precios y planes",
    "Add exercise" => "Añadir ejercicio",
    "FREE" => "GRATIS",
    "ONE DAY TRAINING" => "UN DÍA DE ENTRENAMIENTO",
    "MONTHS MEMBERSHIP" => "MESES DE AFILIACIÓN",
    "Read" => "Leída",
    "read" => "leída",
    "unread" => "no leída",
    "Contact" => "Contacto",
    "Telephone contact" => "Teléfono de contacto",
    "Upload product" => "Subir producto",
    "My statistics" => "Mis estadísticas",
    "Routines" => "Rutinas",
    "Address" => "Dirección",
    "New" => "Nuevo",
    "new notification" => "notificación nueva",
    "Add Users to notification" => "Añadir usuarios a la notificación",
    "Show only my notifications" => "Mostrar sólo mis notificaciones",
    "new notifications" => "notificaciones nuevas",
    "Notification" => "Notificación",
    "Notifications" => "Notificaciones",
    "Add notification" => "Añadir notificación",
    "Modify notification" => "Modificar notificación",
    "Mark as" => "Marcar como",
    "Author" => "Autor",
    "Title" => "Título",
    "Content" => "Contenido",
    "Expiration" => "Caducidad",
    "Receivers" => "Destinatarios",
    "Actives" => "Activas",
    "Lapsed" => "Expiradas",
    "All" => "Todas",
    "Sessions" => "Sesiones",
    "Pause" => "Pausar",
    "Reset" => "Borrar",
    "Save the session?" => "¿Guardar la sesión?",
    "Table" => "Tabla",
    "Duration" => "Duración",
    "Comment" => "Comentario",
    "Edit session" => "Editar sesión",
    "Resume for add session" => "Resumen de la sesión realizada",
    "Begin" => "Comienzo",
    "Reservation" => "Reservar",
    "by %s" => "por %s",
    "Public info" => "Info pública",
    "Name" => "Nombre",
    "Surname" => "Apellidos",
    "Why Join ?" => "¿Por qué unirse?",
    "Have a question?" => "¿Tiene alguna duda?",
    "Contact info" => "Información de contacto",
    "Send us a message" => "Envíenos un mensaje",
    "Submit your message" => "Enviar su mensaje",
    "Description" => "Descripción",
    "Users" => "Usuarios",
    "User" => "Usuario",
    "Images" => "Imágenes",
    "Workout tables" => "Tablas de entrenamiento",
    "Workout table" => "Tabla de entrenamiento",
    "for" => "para",
    "Print" => "Imprimir",
    "standard" => "estandar",
    "customized" => "personalizada",
    "select up to three images" => "selecciona hasta tres imágenes",
    "Add activity schedule for" => "Añadir horario para",
    "Modify activity schedule" => "Modificar horario",
    "Activity"  => "Actividad",
    "Upload" => "Subir",
    "Actions" => "Acciones",
    "Create article" => "Crear artículo",
    "Hello" => "Hola",
    "Modify public info" => "Modificar información pública",
    "You must write a valid phone number" => "Debe escribir un número de teléfono válido",
    "You must write your email" => "Debe escribir su mail",
    "Are you sure?" => "¿Está usted seguro?",
    "Modify profile" => "Modificar perfil",
    "Place" => "Lugar",
    "Seats" => "Plazas",
    "Type" => "Tipo",
    "Schedules" => "Horarios",
    "Schedules of activity" => "Horarios de la actividad",
    "Number of places available" =>"Número de plazas disponibles",
    "You can see the hours of this activity here" =>"Puedes ver los horarios de esta actividad aquí",
    "Date" => "Fecha",
    "Notify" => "Notificar",
    "Update users" => "Actualizar usuarios",
    "Start date" => "Fecha de inicio",
    "End date" => "Fecha de finalización",
    "End hour is previous or equal to start hour" => "Hora fin es anterior o igual a Hora Inicio",
    "Start hour" => "Hora Inicio",
    "End hour" => "Hora Fin",
    "Activity Schedules for " => "Horarios para la actividad ",
    "successfully added. Please, wait to confirm login." => "añadido satisfactoriamente. Por favor, espere confirmación.",
    "Login \"%s\" successfully added." => "Login \"%s\" añadido satisfactoriamente.",
    "Login must be at least 5 characters length" => "Login debe tener al menos 5 caracteres",
    "Password must be at least 5 characters length" => "Password debe tener al menos 5 caracteres",
    "You must write your name"  => "Debe introducir un  nombre",
    "You must write your email" => "Debe introducir un email",
    "user is not valid" => "Usuario no válido",
    "successfully added." => "añadido satisfactoriamente.",
    "Login already exist" => "Login ya existente",
    "Article \"%s\" successfully deleted." => "Artículo \"%s\" eliminado.",
    "Login is not valid" => "Login no válido",
    "Log in" => "Entra",
    "Login" => "Login",
    "Sign in" => "Iniciar sesión",
    "Sign up" => "Registrarse",
    "Sign off" => "Salir",
    "Send" => "Enviar",
    "Chat with" => "Chatea con",
    "about the product" => "sobre el producto",
    "Username" => "Usuario",
    "Password" => "Contraseña",
    "E-mail" => "E-mail",
    "Email" => "Correo electrónico",
    "Subject" => "Asunto",
    "Message" => "Mensaje",
    "Profile image" => "Imagen de perfil",
    "Not registered yet?" => "¿No estás registrado?",
    "Already registered?" => "¿Ya estás registrado?",
    "Create an account" => "Crea una cuenta",
    "Register" => "Registro",
    "Edit" => "Editar",
    "Delete" => "Eliminar",
    "Create article" => "Crear artículo",
    "chatlines" => "Lineas del chat",
    "Write a comment" => "Escribe un comentario",
    "%s commented..."=>"%s ha comentado...",
    "chatline" => "Chat",
    "do comment" => "comentar",
    "Spanish" => "Español",
    "English" => "Inglés",
    "Select" => "Seleccionar",
  	"Sign out" => "Salir",
  	"My account" => "Mi cuenta",
    "are you sure?" => "¿Estás seguro?",
    "Upload product" => "Subir producto",
  	"❮ Prev" => "❮ Anterior",
  	"Next ❯" => "Siguiente ❯",
    "Write something"=> "Escribe algo",
    "Write message"=> "Escribe un mensaje",
    "Add article"=> "Subir producto",
    "are you sure?" => "¿Estás seguro?",
    "Price" => "Precio",
	  "Loading Data, please wait"=>"Cargando datos, espere por favor",
    "Modify product" => "Modificar producto",
    "Modify" => "Modificar",
    "Are you sure to delete?" => "¿Estás seguro de eliminar?",
    "Yes" => "Si",
    "User type" => "Tipo de usuario",
    "Athlete type" => "Tipo de depostista",
    "Administrator" => "Administrador",
    "Trainer" => "Entrenador",
    "Athlete" => "Deportista",
    "Add activity" => "Añadir actividad",
    "Modify activity" => "Modificar actividad",
    "Place" => "Instalación",
    "Seats" => "Plazas",
    "Image" => "Imagen",
    "Add" => "Añadir",
    "select one image" => "seleccionar una o más",
    "Manage resources" => "Gestionar recursos",
    "View" => "Consultar",
    "Resource" => "Recurso",
    "Resources" => "Recursos",
    "Quantity" => "Cantidad",
    "Modify resource" => "Modificar recurso",
    "Add resource" => "Añadir recurso",
    "Add place" => "Añadir instalación",
    "New resource" => "Nuevo recurso",
    "New place" => "Nueva instalación",
    "Exercise"=> "Ejercicio",
    "Exercises"=> "Ejercicios",
    "Manage exercises" => "Gestión de ejercicios",
    "Repetitions" => "Repeticiones",
    "Number of series" => "Número de series",
    "Number of repetitions" => "Número de repeticiones",
    "Assign workout table" => "Asignar tabla de entrenamiento",
    "Modify exercise" => "Modificar ejercicio",
    "from table" => "de tabla",
    "Edit workout table" => "Editar tabla de entrenamiento",
    "Athlets" => "Deportistas",
		"Asign Table" => "Asignar Tabla",
		"View Tables" => "Ver Tablas",
 		"Pending" => "Pendientes",
		"Individual" => "Individual",
		"In group" => "En grupo",
    "Add standad workout table" => "Añadir tabla de entrenamiento estandard",
		"Add customized workout table" => "Añadir tabla de entrenamiento personalizada",
		"Stretch" => "Estiramiento",
      "Cancel Reservation" => "Cancelar Reserva",
      "Complete activity" => "Aforo Completo",
    "Statistics" => "Estadísticas",
    "Registered athletes" => "Deportistas registrados",
    "Athletes by activity" => "Deportistas por actividad",
    "Assistance by date" => "Asistencia por fecha",
    "Sessions duration by date (minutes)" => "Duración de sesiones por fecha (minutos)",
    "Asigned athletes" => "Deportistas asignados",
    "Exercises by type" => "Ejercicios por tipo",
	  "Assistance" => "Asistencia",
	  "Assistance this week: " => "Asistencia esta semana: ",
	  "All Athlets" => "Todos Deportistas",
      "My Athlets" => "Mis Deportistas",
      "TDU Athlets" => "Deportistas TDU",
	  "View Sessions" => "Ver sesiones",
      "Sessions of " => "Sesiones de "
  )
?>
